...
// uint64 sys_lseek(void) supports only SEEK_SET and SEEK_CUR and SEEK_END
uint64 sys_lseek(void) {
  ...
  switch (whence) {
    case SEEK_SET:
      if (offset > f->ip->size || offset < 0) return -1;
      f->off = offset;
      break;
    case SEEK_CUR:
      if ((f->off + offset) > f->ip->size || (f->off + offset) < 0) return -1;
      f->off += offset;
      break;
    case SEEK_END:
      if ((int)(f->ip->size) + offset < 0 || offset > 0)
        return -1;
