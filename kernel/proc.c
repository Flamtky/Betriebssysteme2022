...
static void freeproc(struct proc *p) {
  ...
  p->trapframe = 0;
  if (p->pagetable) proc_freepagetable(p->pagetable, p->sz, p->stacksize);
  p->pagetable = 0;
  p->sz = 0;
  p->pid = 0;
  p->parent = 0;
...
void proc_freepagetable(pagetable_t pagetable, uint64 sz, uint64 stacksize) {
  uvmunmap(pagetable, TRAMPOLINE, 1, 0);
...
int fork(void) {
  ...
  np->sz = p->sz;
  if (uvmcopy(p->pagetable, np->pagetable, USTACKTOP - p->stacksize,
              USTACKTOP) < 0) {
    freeproc(np);
    release(&np->lock);
    return -1;
  }
  np->stacksize = p->stacksize;  // Copy stack size.
